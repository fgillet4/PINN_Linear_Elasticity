...
def compute_loss_with_l2(model, l2_lambda):
    # Physics loss
    ...
    return total_loss, physics_loss, l2_loss
...
def run_l2_experiment(l2_lambda, experiment_name, num_epochs=5000):
    ...
    for i in range(num_epochs + 1):
        optimizer.zero_grad()
        total_loss, physics_loss, l2_loss = compute_loss_with_l2(model, l2_lambda)
        total_loss.backward()
        optimizer.step()
        
        if i == 0:
            total_loss0 = total_loss.item()
            physics_loss0 = physics_loss.item()
        
        hist_total.append(total_loss.item() / total_loss0)
        hist_physics.append(physics_loss.item() / physics_loss0)
        hist_l2.append(l2_loss.item())
        
        if i % 200 == 0:
            print('It {:05d}: total_loss = {:10.8e}, physics_loss = {:10.8e}, l2_loss = {:10.8e}'.format(
                i, total_loss.item(), physics_loss.item(), l2_loss.item()))
    ...
    return hist_total, hist_physics, hist_l2, model, training_time
...
for l2_lambda, exp_name in zip(l2_lambdas, experiment_names):
    hist_total, hist_physics, hist_l2, model, training_time = run_l2_experiment(l2_lambda, exp_name)
    results[exp_name] = {
        'l2_lambda': l2_lambda,
        'hist_total': hist_total,
        'hist_physics': hist_physics,
        'hist_l2': hist_l2,
        'model': model,
...
for exp_name, result in results.items():
    axes[0].semilogy(range(len(result['hist_total'])), result['hist_total'], 
                     label=f"{exp_name} (λ={result['l2_lambda']})")
axes[0].set_xlabel('Epoch')
axes[0].set_ylabel('Normalized Total Loss')
...
for exp_name, result in results.items():
    axes[1].semilogy(range(len(result['hist_physics'])), result['hist_physics'], 
                     label=f"{exp_name} (λ={result['l2_lambda']})")
axes[1].set_xlabel('Epoch')
axes[1].set_ylabel('Normalized Physics Loss')
...
for exp_name, result in results.items():
    final_total = result['hist_total'][-1] * (result['hist_total'][0] if len(result['hist_total']) > 0 else 1)
    final_physics = result['hist_physics'][-1] * (result['hist_physics'][0] if len(result['hist_physics']) > 0 else 1)
