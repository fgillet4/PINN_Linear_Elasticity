...
def compute_detailed_loss(model):
...
def run_lr_experiment(schedule_type, schedule_params, experiment_name, num_epochs=5000):
    ...
    for i in range(num_epochs + 1):
        optimizer.zero_grad()
        loss_components = compute_detailed_loss(model)
    ...
    return epochs, residual_history, hist_lr, model, training_time
...
schedules = {
    ...
    'reduce_plateau': {
        'type': 'reduce_plateau',
        'params': {'initial_lr': 1e-2, 'factor': 0.5, 'patience': 500},
...
for key, schedule in schedules.items():
    epochs, residual_history, hist_lr, model, training_time = run_lr_experiment(
        schedule['type'], 
        schedule['params'], 
        schedule['name']
    )
...
for key, result in results.items():
    axes[0, 0].semilogy(result['epochs'], result['residuals']['total_loss'], 
...
for key, result in results.items():
    epochs_arr = np.array(result['epochs'])
    loss_arr = np.array(result['residuals']['total_loss'])
    mask = epochs_arr >= (epochs_arr[-1] - 1000)
    axes[1, 0].semilogy(epochs_arr[mask], loss_arr[mask], 
                        label=f"{result['schedule']['name']}")
...
for key, result in results.items():
    loss_arr = np.array(result['residuals']['total_loss'])
    loss_diff = np.diff(loss_arr)
    axes[1, 1].plot(result['epochs'][1:], -loss_diff, 
...
for key, result in results.items():
    final_loss = result['residuals']['total_loss'][-1]
    min_loss = min(result['residuals']['total_loss'])
...
print("- Residual data: residuals_[schedule_name].npy")