...
# Noise level (percentage of signal)
NOISE_LEVEL = 0.05
print(f"Adding {NOISE_LEVEL*100}% noise to training data")
...
noise_eux_up = NOISE_LEVEL * torch.randn_like(eux_up_train)
noise_eux_lo = NOISE_LEVEL * torch.randn_like(eux_lo_train)
noise_uy_lo = NOISE_LEVEL * torch.randn_like(uy_lo_train)
noise_uy_ri = NOISE_LEVEL * torch.randn_like(uy_ri_train)
noise_uy_le = NOISE_LEVEL * torch.randn_like(uy_le_train)
noise_Sxx_ri = NOISE_LEVEL * torch.randn_like(Sxx_ri_train) * 10.0
noise_Sxx_le = NOISE_LEVEL * torch.randn_like(Sxx_le_train) * 10.0
noise_Syy_up = NOISE_LEVEL * torch.randn_like(Syy_up_train) * 10.0

eux_up_train_noisy = eux_up_train + noise_eux_up
eux_lo_train_noisy = eux_lo_train + noise_eux_lo
uy_lo_train_noisy = uy_lo_train + noise_uy_lo
uy_ri_train_noisy = uy_ri_train + noise_uy_ri
uy_le_train_noisy = uy_le_train + noise_uy_le
Sxx_ri_train_noisy = Sxx_ri_train + noise_Sxx_ri
Sxx_le_train_noisy = Sxx_le_train + noise_Sxx_le
Syy_up_train_noisy = Syy_up_train + noise_Syy_up

# Combine boundary data using noisy BCs
eux_b_train = torch.cat([eux_up_train_noisy, eux_lo_train_noisy], 0)
uy_b_train = torch.cat([uy_lo_train_noisy, uy_ri_train_noisy, uy_le_train_noisy], 0)
Sxx_b_train = torch.cat([Sxx_ri_train_noisy, Sxx_le_train_noisy], 0)
Syy_b_train = Syy_up_train_noisy

# Collocation points (no noise added to physics equations)
grid_pt = lhs(2, N_r)
...
print("Generating validation points (no noise)...")
N_bound_val = 25
N_r_val = 500
...
print("Starting training with noisy data...")
N = 10000
train_hist = []
val_hist = []
t0 = time()
...
U_total = [Ux, Uy]
U_total_name = ['Ux_NN_PyTorch_Noisy', 'Uy_NN_PyTorch_Noisy']
...
fig.savefig('results/solution_pytorch_with_noise.png')
...
error_total_name = ['point wise error Ux (with noise)', 'point wise error Uy (with noise)']
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 6))
for i, ax in enumerate(axes.flat):
...
fig.colorbar(im, ax=axes.ravel().tolist())
fig.savefig('results/error_map_pytorch_with_noise.png')

# Plot stress components
S_total = [Sxx, Syy, Sxy]
S_total_name = ['Sxx_NN_PyTorch_Noisy', 'Syy_NN_PyTorch_Noisy', 'Sxy_NN_PyTorch_Noisy']
...
fig.colorbar(im, ax=axes.ravel().tolist())
fig.savefig('results/stress_map_pytorch_with_noise.png')

# Plot loss history with training and validation
fig = plt.figure(figsize=(9, 6))
ax = fig.add_subplot(111)
ax.semilogy(range(len(train_hist)), train_hist, 'b-', label='Training Loss (with noise)')
ax.semilogy(range(len(val_hist)), val_hist, 'r-', label='Validation Loss (no noise)')
ax.set_xlabel('$n_{epoch}$')
ax.set_ylabel('$\\phi_{n_{epoch}}$')
ax.set_title(f'Training with {NOISE_LEVEL*100}% Noise in Boundary Conditions')
ax.legend()
ax.grid(True, alpha=0.3)
fig.savefig('results/loss_history_pytorch_with_noise.png')

# Save model
torch.save(model.state_dict(), 'solidmechanics_model_pytorch_with_noise.pt')
print(f"PyTorch PINN training with {NOISE_LEVEL*100}% noise completed successfully!")
